...
Controls:
 ...
 - Reverse: LEFT-hand pinch (thumb <-> index) -> DOWN (reverse) with same partial/full rules
 - Brake: RIGHT-hand closed fist -> hold DOWN (brake) [unchanged]
...
KEY_RIGHT = 'right'
KEY_ACCEL = 'up'
KEY_REVERSE = 'down'   # reverse uses 'down' arrow
KEY_BRAKE = 'down'     # brake was mapped to down in original code — keep as is
...
def pinch_distance(hand_landmarks):
    ...
    try:
        ...
        return normalized_distance_pts(thumb, idx)
    except Exception:
        return 1.0
    ...
    while True:
        ...
        if results.multi_hand_landmarks and len(results.multi_hand_landmarks) >= 2:
            ...
            if w0 is not None and w1 is not None:
                ...
                # ********** PINCH HANDLING FIXED HERE **********
                # compute pinch distance for both hands (smaller -> stronger pinch)
                pinch_right = pinch_distance(right_hand)
                pinch_left = pinch_distance(left_hand)

                # We will decide forward/reverse based on which hand has the stronger pinch
                # (smaller distance). This avoids sending both UP and DOWN simultaneously.
                # If neither hand is pinching enough, we coast.
                chosen_action = None  # 'forward' or 'reverse' or None
                chosen_interval = None

                # Helper to compute partial-press interval from a pinch value
                def pinch_to_interval(p):
                    norm = (PINCH_PARTIAL_ACCEL - p) / (PINCH_PARTIAL_ACCEL - PINCH_FULL_ACCEL) if (PINCH_PARTIAL_ACCEL - PINCH_FULL_ACCEL) != 0 else 0.0
                    norm = max(0.0, min(1.0, norm))
                    return PARTIAL_PRESS_MIN_INTERVAL + (1.0 - norm) * (PARTIAL_PRESS_MAX_INTERVAL - PARTIAL_PRESS_MIN_INTERVAL)

                # Check full thresholds first
                right_full = pinch_right < PINCH_FULL_ACCEL
                left_full = pinch_left < PINCH_FULL_ACCEL

                # If both full, pick the stronger (smaller distance)
                if right_full or left_full:
                    if right_full and left_full:
                        # pick stronger pinch
                        if pinch_right <= pinch_left:
                            chosen_action = 'forward'
                        else:
                            chosen_action = 'reverse'
                    elif right_full:
                        chosen_action = 'forward'
                    else:
                        chosen_action = 'reverse'
                    ...
                    # check partial zones
                    right_partial = pinch_right < PINCH_PARTIAL_ACCEL
                    left_partial = pinch_left < PINCH_PARTIAL_ACCEL

                    if right_partial or left_partial:
                        if right_partial and left_partial:
                            # pick stronger pinch for partial as well
                            if pinch_right <= pinch_left:
                                chosen_action = 'forward'
                                chosen_interval = pinch_to_interval(pinch_right)
                            else:
                                chosen_action = 'reverse'
                                chosen_interval = pinch_to_interval(pinch_left)
                        elif right_partial:
                            chosen_action = 'forward'
                            chosen_interval = pinch_to_interval(pinch_right)
                        else:
                ...
                if chosen_action == 'forward':
                    # full vs partial
                    if pinch_right < PINCH_FULL_ACCEL:
                        camera_desired_keys.add(KEY_ACCEL)
                        forward_state = "Full (R pinch)"
                    else:
                        # partial: pulse based on interval
                        if chosen_interval is None:
                            chosen_interval = pinch_to_interval(pinch_right)
                        if time.time() - last_partial_time > chosen_interval:
                            robust_tap(KEY_ACCEL)
                            last_partial_time = time.time()
                        forward_state = f"Partial (R, {chosen_interval:.2f}s)"
                    # ensure reverse is not sent
                    reverse_state = "Coast"
                elif chosen_action == 'reverse':
                    # use KEY_REVERSE (down)
                    if pinch_left < PINCH_FULL_ACCEL:
                        ...
                        if time.time() - last_partial_time > chosen_interval:
                            ...
                            last_partial_time = time.time()
                        reverse_state = f"Partial (L, {chosen_interval:.2f}s)"
                    # ensure forward not sent
                    forward_state = "Coast"
                else:
                    forward_state = "Coast"
                    reverse_state = "Coast"

                # Right-hand fist -> brake (hold DOWN) — keep behavior unchanged
                if is_fist(right_hand):
                    camera_desired_keys.add(KEY_BRAKE)
                    brake_state = "Brake (Right fist)"
                else:
                    # if brake was added by camera_desired_keys earlier, it will be released later if not desired
                    brake_state = "No"
            ...
            if results.multi_hand_landmarks and len(results.multi_hand_landmarks) == 1:
                ...
                pinch_val = pinch_distance(single)
                if pinch_val < PINCH_FULL_ACCEL:
                    camera_desired_keys.add(KEY_ACCEL)
                    forward_state = "Full(single)"
                    reverse_state = "Coast(single)"
        ...
        try:
            ...
            if not camera_desired_keys:
                ...
                if 'down' in normalized or KEY_REVERSE in normalized:
        ...
        try:
            ...
            cv2.putText(frame, f"Forward: {forward_state}", (8, 38), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1)
            cv2.putText(frame, f"Reverse: {reverse_state}", (8, 58), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1)
            cv2.putText(frame, f"Brake: {brake_state}", (8, 78), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (255,255,255), 1)
            cv2.putText(frame, f"Pinch R: {pinch_right:.3f}", (8, 98), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200,200,200), 1)
            cv2.putText(frame, f"Pinch L: {pinch_left:.3f}", (8, 118), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200,200,200), 1)
            cv2.putText(frame, f"Tilt rad: {tilt_angle:.3f}", (8, 138), cv2.FONT_HERSHEY_SIMPLEX, 0.45, (200,200,200), 1)
            ...
            cv2.putText(frame, f"Camera-held: {camkeys}", (8, 158), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (180,180,255), 1)
            cv2.putText(frame, f"Phys-held: {physkeys}", (8, 176), cv2.FONT_HERSHEY_SIMPLEX, 0.38, (180,255,180), 1)
        except Exception as e:
